    ...
    <div class="bg-white rounded-xl shadow-sm border p-8 space-y-8">
        ...
        <div>
            ...
            <div id="clientLogoPreview"
                ...
                @if($quotation->client_logo)
                    <img src="{{ asset('storage/' . $quotation->client_logo) }}" class="w-full h-full object-cover">
                @else
            ...
            <button type="button" onclick="document.getElementById('clientLogoInput').click()"
        ...
        <div class="bg-gray-50 border rounded-md p-4 grid sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="date_issued"
                       value="{{ old('date_issued', $quotation->date_issued->format('Y-m-d')) }}" required
                       class="w-full border mt-1 rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500">
            </div>
            <div>
                ...
                <table class="w-full text-left text-sm border rounded-lg">
                    ...
                    <tbody id="itemRows" class="divide-y">
                        @foreach($quotation->details as $i => $detail)
                            <tr>
                                <td class="px-4 py-3"><input type="text" name="items[{{ $i }}][name]" value="{{ $detail->item_name }}" class="w-full border rounded-md px-2 py-1 text-sm"></td>
                                <td class="px-4 py-3"><textarea rows="2" name="items[{{ $i }}][description]" class="w-full border rounded-md px-2 py-1 text-sm">{{ $detail->description }}</textarea></td>
                                <td class="px-4 py-3 w-20"><input type="number" name="items[{{ $i }}][quantity]" value="{{ $detail->quantity }}" min="1" step="1" class="item-qty border rounded-md px-2 py-1 w-full text-center text-sm"></td>
                                <td class="px-4 py-3 w-24"><input type="number" name="items[{{ $i }}][unit_price]" value="{{ $detail->unit_price }}" min="0" step="0.01" class="item-price border rounded-md px-2 py-1 w-full text-center text-sm"></td>
                                <td class="px-4 py-3 text-gray-700 item-total">₱{{ number_format($detail->total, 2) }}</td>
                                <td class="px-4 py-3 text-right"><button type="button" class="remove-row text-red-500 hover:text-red-700 text-sm">Remove</button></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="overflow-x-auto border rounded-lg bg-gray-50">
                <table class="w-full text-sm text-left border-collapse">
                    ...
                    <tbody id="scopeScenarioRows" class="divide-y">
                        @foreach($quotation->scopes as $sIndex => $scope)
                            <tr class="scenario-row">
                                ...
                                <td colspan="2" class="px-4 py-3">
                                    <table class="w-full text-sm border bg-white rounded-md case-table mb-2">
                                        <tbody>
                                            @foreach($scope->cases as $cIndex => $case)
                                                <tr class="case-row">
                                                    <td class="px-4 py-2 w-1/4">
                                                        <input type="text" name="scope[{{ $sIndex }}][cases][{{ $cIndex }}][name]" value="{{ $case->case_title }}" class="w-full border rounded-md px-2 py-1 text-sm">
                                                    </td>
                                                    <td class="px-4 py-2">
                                                        <textarea rows="2" name="scope[{{ $sIndex }}][cases][{{ $cIndex }}][description]" class="w-full border rounded-md px-2 py-1 text-sm">{{ $case->case_description }}</textarea>
                                                    </td>
            ...
            <div class="overflow-x-auto border rounded-lg bg-gray-50">
                <table class="w-full text-sm text-left border-collapse">
                    ...
                    <tbody id="waiverScenarioRows" class="divide-y">
                        @foreach($quotation->waivers as $wIndex => $waiver)
                            <tr class="waiver-scenario-row">
                                <td class="px-4 py-3 font-semibold text-gray-800">
                                ...
                                <td colspan="2" class="px-4 py-3">
                                    <table class="w-full text-sm border bg-white rounded-md waiver-case-table mb-2">
                                        <tbody>
                                            @foreach($waiver->cases as $cIndex => $case)
                                                <tr class="waiver-case-row">
                                                    <td class="px-4 py-2 w-1/4">
                                                        <input type="text" name="waiver[{{ $wIndex }}][cases][{{ $cIndex }}][name]" value="{{ $case->case_title }}" class="w-full border rounded-md px-2 py-1 text-sm">
                                                    </td>
                                                    <td class="px-4 py-2">
                                                        <textarea rows="2" name="waiver[{{ $wIndex }}][cases][{{ $cIndex }}][description]" class="w-full border rounded-md px-2 py-1 text-sm">{{ $case->description }}</textarea>
                                                    </td>
                                                    <td class="px-4 py-2 text-right">
        ...
        <div>
            ...
            <div class="overflow-x-auto bg-gray-50 border rounded-lg">
                <table class="w-full text-sm text-left border-collapse">
                    ...
                    <tbody id="deliverableRows" class="divide-y">
                        @foreach($quotation->deliverables as $dIndex => $deliverable)
                            <tr>
                                <td class="px-4 py-3 text-gray-600">{{ $dIndex + 1 }}</td>
                                <td class="px-4 py-3">
                                    <input type="text" name="deliverables[{{ $dIndex }}][detail]" value="{{ $deliverable->deliverable_detail }}" class="w-full border rounded-md px-2 py-1 text-sm">
                                </td>
                                <td class="px-4 py-3 text-right">
        ...
        <div>
            ...
            <div class="flex items-center gap-2">
                <input id="minDays" name="timeline_min_days" type="number" value="{{ old('timeline_min_days', $quotation->timeline_min_days) }}" class="w-24 border rounded-md px-3 py-2 text-sm">
                <span class="text-gray-500">to</span>
                <input id="maxDays" name="timeline_max_days" type="number" value="{{ old('timeline_max_days', $quotation->timeline_max_days) }}" class="w-24 border rounded-md px-3 py-2 text-sm">
        ...
        <div>
            ...
            <textarea rows="5" name="terms_conditions" class="w-full border rounded-md px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500">{{ old('terms_conditions', $quotation->terms_conditions) }}</textarea>
        </div>

        <div class="flex justify-end space-x-3 border-t pt-4">
            <a href="{{ route('technician.quotation') }}">
                <button type="button" class="px-4 py-2 border rounded-md text-gray-600 text-sm hover:bg-gray-100">Cancel</button>
            </a>
            <button type="submit" name="action" value="save" class="px-4 py-2 bg-green-600 text-white rounded-md text-sm hover:bg-green-700">Update Quotation</button>
        </div>
    </div>
</form>

<!-- ✅ Reuse the entire JavaScript block from create.blade.php -->
<script>
// ============================================
// UTILITY FUNCTIONS
// ============================================
